...
namespace FileSys 
{
    ...
    namespace {
        std::time_t FileTimeToTimeT(const std::filesystem::file_time_type& ftime)
        {
    ...
    std::time_t GetLastWriteTime(const std::string& path)
    {
        ...
        return FileTimeToTimeT(std::filesystem::last_write_time(path));
    }

    std::time_t GetCreationTime(const std::string& path)
    {
#if defined(_WIN32)
        ...
        return attr.st_birthtime;
    #elif defined(__linux__)
        // Linux ext4 usually doesn't store creation time.
        // We return last modification as a fallback.
    ...
    std::time_t GetLastAccessTime(const std::string& path)
    {
        struct stat attr;
        if (stat(path.c_str(), &attr) != 0)
        {
